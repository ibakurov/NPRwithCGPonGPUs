...
public class DeviationFromNormal extends FitnessFunction {
    ...
    public double getRaw ( Response r, BufferedImage img,
        ...
        for ( int i = 0; i < r.HISTOGRAM.length; i++ ) {
            ...
            if (total == Float.NEGATIVE_INFINITY || total == Float.POSITIVE_INFINITY || total == Float.NaN) {
            	break;
            }
        ...
        //total *= 1000.0;
        if ( total <= 0.0000001 || total == Float.NEGATIVE_INFINITY || total == Float.POSITIVE_INFINITY || total == Float.NaN) return 100.0;
        return total;
