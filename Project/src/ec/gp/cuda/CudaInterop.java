...
//    private static final String T_KERNEL_OUT_TYPE_NOPOINTER = "/*@@kernel-out-type-nopointer@@*/";
	...
	protected String outputFilename = "src/ec/gp/cuda/kernel/evaluator.cu";
...
//		byte[][] population = new byte[3][expCount * maxExpLength];
//		int i = 0;
//
//		for (List<TByteArrayList> thExps : expressions) {
//			for (int k = 0; k < thExps.size(); k++) {
//				TByteArrayList currExp = thExps.get(k);
//				int length = currExp.size();
//				currExp.toArray(population[k], 0, i * maxExpLength, length);
//				i++;
//			}
//		}
		...
		KernelOutputData[] results = kernelInfo.instantiateOutputArray(state, problemSize*3, 1/*expCount/*((MainRecord)data).x.length*/);
...
//			int thisPopShare;
			...
			if (i == gpuCount - 1) {
				thisOutputShare = expCount - i * (expCount / gpuCount);
			}
			else {
				thisOutputShare = expCount / gpuCount;
			}
...
//					devExpression.free();
			...
			kernelJob.gridDimX = problemSize / evaluateBlockSize + 1;
			...
			kernelJob.argSetter = new KernelArgSetter() {
				...
				public Pointer getArgs() {
					 ...
					 * 
					 */
					Pointer[] kernelArguments = new Pointer[userArgsCount + /*6*/5];
...
//					kernelArguments[userArgsCount++] = devExpression.toPointer();	// This GPU's portion of the expression
					kernelArguments[userArgsCount++] = Pointer.to(new int[] {problemSize / evaluateBlockSize + 1});	// This GPU's portion of the output
					kernelArguments[userArgsCount++] = Pointer.to(new int[] {problemSize});	// This GPU's portion of the output
//					kernelArguments[userArgsCount] = Pointer.to(new int[] {maxExpLength});	// Maximum length of each expression
			...
			kernelJob.id = "expressions: " + expCount + "; pixels: " + problemSize /*+ " (array length of  " + population.length + ") my share is " + thisOutputShare + " processing " + thisPopShare + " maxExpLength:" + maxExpLength*/;
...
//		public int myShare;		// How many individuals am I handling?
		...
		public void run() {
			...
			for (int i = 0 ; i < destination.length ; i++) {
				...
				item.copyValues(kernelResult, i * problemSize, problemSize*3);
			}
		}
	...
	@Override
	public void setup(EvolutionState arg0, Parameter arg1) {
