...
public class ColorHistogram {
    ...
    public ColorHistogram ( BufferedImage img ) {
        ...
        int start = 0;
        int inc = 1;//img.getHeight();// / param.Parameters.THREADS;

        HistThread fit[] = new HistThread[1];

        for (int i = 0; i < 1; i++) {
            fit[0] = new HistThread(img, start, start + inc, data.clone());
            start = start + inc;
            fit[0].start();
        }
